...
module.exports = async (client, interaction) => {
    if (!cachedLocalCommands) {
        cachedLocalCommands = getLocalCmds(join(__dirname, "..", "..", "commands"));
    }
    const localCommands = cachedLocalCommands;

    if (!cachedUserPerms) {
        const [perms] = await userpermissions();
        cachedUserPerms = perms;
    }
    const userperms = cachedUserPerms;
