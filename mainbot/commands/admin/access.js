...
const { invoicesMap, INVOICE_STATUS } = require("../../utils/purchase/combined");
...
module.exports = {
  ...
  async autocomplete(client, interaction) {
    try {
      ...
      const isUserOwner = owners && Array.isArray(owners) && owners.includes(interaction.user.id);
      ...
      if (subcommand === 'access') {
        ...
        return interaction.respond([
          { name: "---Manage Keys---", value: "placeholder1"},
          ...
          { name: "---Remove Keys---", value: "placeholder2"},
          ...
          { name: "---View Lists---", value: "placeholder3"},
          ...
          { name: "---Invoices---", value: "placeholder4"},
          { name: "Mark Invoice Paid (manual)", value: "mark_paid"},
          { name: "View Active Invoices", value: "view_invoices"}
        ]);
      ...
      console.error("Autocomplete error in access command:", error);
      if (!interaction.responded) {
      ...
      try {
        ...
        switch (option) {
          ...
          case "key_redeemed":
            ...
            break;
          case 'mark_paid':
            await handleMarkPaid(client, interaction);
            break;
          case 'view_invoices':
            await handleViewInvoices(client, interaction);
            break;
          default:
...
// ---------------------------------------------------------------
// Mark Invoice Paid (manual override)
// ---------------------------------------------------------------
async function handleMarkPaid(client, interaction) {
  await interaction.deferReply({ ephemeral: true });

  // Accept invoice ID via 'key' option, or look up by user_id
  const invoiceId = interaction.options.getString('key') || null;
  const userId    = interaction.options.getString('user_id') || null;

  if (!invoiceId && !userId) {
    return interaction.editReply({
      content: '❌ Please provide an Invoice ID (via the `key` field) or a `user_id` to look up their pending invoice.'
    });
  }
  ...
  if (invoiceId) {
    invoice = invoicesMap.get(invoiceId);
    if (!invoice) {
      return interaction.editReply({
        content: `❌ No active invoice found with ID \`${invoiceId}\`. It may have expired or already been processed.`
      });
    }
  } else {
    // Find the first PENDING invoice belonging to this user
    for (const [, inv] of invoicesMap) {
      if (inv.user_id === userId && inv.status === INVOICE_STATUS.PENDING) {
        invoice = inv;
        break;
      }
    }
    if (!invoice) {
      return interaction.editReply({
        content: `❌ No pending invoice found for <@${userId}>. They may not have an active purchase thread.`
      });
    }
  }
  ...
  if (invoice.status === INVOICE_STATUS.PAID) {
    return interaction.editReply({
      content: `⚠️ Invoice \`${invoice.invoiceId}\` is already marked as **PAID** (delivery status: **${invoice.deliveryStatus}**).`
    });
  }

  if (invoice.status === INVOICE_STATUS.CLOSED) {
    return interaction.editReply({
      content: `⚠️ Invoice \`${invoice.invoiceId}\` is **CLOSED** and can no longer be processed.`
    });
  }
  ...
  try {
    ...
    console.error('[mark_paid] deliverProduct error:', err);
    return interaction.editReply({
      content: `❌ Invoice marked paid but product delivery failed: \`${err.message}\`. Please deliver the key manually.`
    });
  }

  // Archive the purchase thread if possible
  if (invoice.thread_id) {
    try {
      const thread = await client.channels.fetch(invoice.thread_id).catch(() => null);
      if (thread && thread.isThread()) {
        await thread.send({
          content: `✅ Payment manually confirmed by <@${interaction.user.id}>. Your key has been delivered! This thread will now be archived.`
        });
        setTimeout(() => thread.setArchived(true, 'Invoice manually marked paid by admin').catch(() => {}), 3000);
      }
    } catch (e) {
      console.error('[mark_paid] Could not archive thread:', e);
    }
  }

  // Log it
  console.log(`[mark_paid] Invoice ${invoice.invoiceId} marked PAID by ${interaction.user.id} (buyer: ${invoice.user_id})`);

  return interaction.editReply({
    content:
      `✅ Invoice \`${invoice.invoiceId}\` has been manually marked as **PAID**.\n` +
      `• Buyer: <@${invoice.user_id}>\n` +
      `• Product: \`${invoice.product_type || 'license'}\`\n` +
      `• Key delivered to DM${invoice.thread_id ? ' and purchase thread' : ''}.`
  });
}
...
// ---------------------------------------------------------------
async function handleViewInvoices(client, interaction) {
  await interaction.deferReply({ ephemeral: true });

  if (invoicesMap.size === 0) {
    return interaction.editReply({ content: '📭 There are no active invoices right now.' });
  }

  const lines = [];
  for (const [id, inv] of invoicesMap) {
    const expiresIn = inv.availableUntil
      ? `<t:${Math.floor(inv.availableUntil / 1000)}:R>`
      : 'unknown';
    lines.push(
      `**ID:** \`${id}\`\n` +
      `  • Buyer: <@${inv.user_id}>\n` +
      `  • Product: \`${inv.product_type || 'license'}\`\n` +
      `  • Status: \`${inv.status}\` | Delivery: \`${inv.deliveryStatus}\`\n` +
      `  • Expires: ${expiresIn}\n` +
      `  • LTC Amount: \`${inv.price ? inv.price.toFixed(8) : 'N/A'}\` LTC`
    );
  }
  ...
  if (fullText.length <= chunkSize) {
    return interaction.editReply({ content: `📋 **Active Invoices (${invoicesMap.size}):**\n\n${fullText}` });
  }

  // Split into chunks if too long
  await interaction.editReply({ content: `📋 **Active Invoices (${invoicesMap.size}) — showing first entries:**\n\n${fullText.slice(0, chunkSize)}` });
}
